$.fn.select2.defaults.set('width', '32%');
...
{
	cmbProvince = $("#"+_cmbProvince);
	cmbCity = $("#"+_cmbCity);
	cmbArea = $("#"+_cmbArea);
		setProvince("请选择（省）");
		setCity(provinceList[0].cityList,"请选择（市）");
		setArea(provinceList[0].cityList[0].areaList,"请选择（区）");
	document.getElementById(_cmbProvince).onchange = province_change;
	document.getElementById(_cmbCity).onchange = city_change;
	document.getElementById(_cmbArea).onchange = area_change;
}
function setProvince(str){
	var provincedata = new Array();
	for(var i=0; i<provinceList.length; i++){
		provincedata.push({id:i,text:provinceList[i].name,obj:provinceList[i].cityList});
	}
	cmbProvince.select2({
		 data: provincedata,
		 placeholder:str,
...
function setCity(array,str){
	var citydata = new Array();
	for(var i=0; i<array.length; i++){
		citydata.push({id:i,text:array[i].name,obj:array[i].areaList});
	}
	cmbCity.select2({
		 data: citydata,
		 placeholder:str,
...
function setArea(array,str){
	var areadata = new Array();
	for(var i=0; i<array.length; i++){
		areadata.push({id:i,text:array[i]});
	}
	cmbArea.select2({
		 data: areadata,
		 placeholder:str,
...
function province_change(){
	province_num = cmbProvince.val();
	if(province_num == ""){
		province_num = 0;
	}
	province_num = parseInt(province_num);
	province_str = provinceList[province_num].name;
	cmbCity.val("");
	setCity(provinceList[province_num].cityList,provinceList[province_num].cityList[0].name);
	city_change();
}
var city_num ;
var city_str ;
function city_change(){
	city_num = cmbCity.val();
	if(city_num == ""){
		city_num = 0;
	}
	city_num = parseInt(city_num);
	city_str = provinceList[province_num].cityList[city_num].name;
	cmbArea.val("");
	setArea(provinceList[province_num].cityList[city_num].areaList,provinceList[province_num].cityList[city_num].areaList[0]);
	area_change();
}
var area_num ;
var area_str ;
function area_change(){
	area_num = cmbArea.val();
	if(area_num == ""){
		area_num = 0;
	}
	area_num = parseInt(area_num);
	area_str = provinceList[province_num].cityList[city_num].areaList[area_num];
}
function getAddress(){
	var array = [province_str,city_str,area_str];
	return array;
